...
export default function RandomCharacter(props) {
  ...
  const toggleRandom = (oldRandomIndex) => {
    ...
    var delayToNextChar = (charType !== "romaji") ? 1000 : 1230;
    setCanRandom(true);
    ...
    if (soundEnable && randomIndex > -1 && (charType === "hiragana" || charType === "katakana")) {
      playSound(characterTran);
    }
  ...
  const displayChar = (oldRandom) => {
    ...
    if (soundEnable && charType === "romaji") {
      playSound(characterData[randomRun]["romaji"]);
    }
    ...
    setCharacterText(
      (charType !== "romaji")
        ? characterData[randomRun]["character"]
        : characterData[randomRun]["romaji"]
    );
    setCharacterTran(
      (charType !== "romaji")
        ? characterData[randomRun]["romaji"]
  ...
  React.useEffect(() => {
    ...
    if (!charData) {
      ...
      const soundsData = japannese.preloadSound(charData);
      // console.log(soundsData)
      setSoundsTrack(soundsData);
  ...
  return (
    <Grid container spacing={2}>
      ...
      {
        (charType !== "romaji") || (charType === "romaji" && !soundEnable)
          ? <Grid item xs={12} >
            <Button
              size="large"
            ...
            <Grid container spacing={2}>
              <Grid item sm={6} xs={12} >
                <Button
                  ...
                  color="success"
                  variant="contained"
                  onClick={() => toggleReplaySound()}
                  disabled={!soundKeyForReplay}
                >
                  เล่นเสียง
                </Button>
              </Grid>
